...
from common import (
    BASE_DIR, KUAN_COLLOQUIAL, NATURE_MAP, SETSU_TO_NATURE, NATURE_COLORS,
    INDEPENDENT_REVENUE, DEPENDENT_REVENUE, KUAN_COLORS, COLOR_INCREASE,
    COLOR_DECREASE, SRC_COLORS, POPULATION, FURUSATO_COST_ITEMS,
    FURUSATO_FUND_SIM, GLOSSARY,
    safe_pct, fmt_oku, fmt_diff, kuan_with_colloquial, classify_revenue, tip,
)

DB_PATH = os.path.join(BASE_DIR, "budget_r8.db")
...
def render():
    ...
    with tab1:
        ...
        for _, r in _pc_bar.iterrows():
            fig_pc.add_trace(go.Bar(
                x=[r["per_capita"]], y=["1人あたり"], orientation="h",
        ...
        fig_pc.update_layout(barmode="stack", height=100, margin=dict(l=0, r=0, t=0, b=0),
        ...
        fig_bar = px.bar(chart_data, x="amount_current", y="kuan_display", orientation="h",
                         color="kuan", color_discrete_map=kuan_color_map, text="label")
        fig_bar.update_layout(height=max(350, len(chart_data) * 36), showlegend=False,
                              margin=dict(l=0, r=60, t=10, b=10),
                              xaxis=dict(visible=False), yaxis=dict(title="", categoryorder="total ascending"))
        fig_bar.update_traces(textposition="outside", textfont_size=13, cliponaxis=False,
        ...
        tree_data["label"] = tree_data.apply(lambda r: f"{r['kuan']}<br>{fmt_oku(int(r['amount_current']))}", axis=1)
        fig_tree = px.treemap(tree_data, path=["kuan"], values="amount_current", color="kuan",
                              color_discrete_map=kuan_color_map)
        fig_tree.update_layout(height=420, margin=dict(l=10, r=10, t=10, b=10))
        ...
        fig_comp = px.bar(comp_data, x="diff", y="kuan", orientation="h", color="color",
                          color_discrete_map={"増加": COLOR_INCREASE, "減少": COLOR_DECREASE}, text="label")
        fig_comp.update_layout(height=max(350, len(comp_data) * 36), margin=dict(l=0, r=70, t=10, b=10),
                               xaxis=dict(visible=False), yaxis=dict(title=""), legend_title_text="",
                               legend=dict(orientation="h", yanchor="bottom", y=-0.08, xanchor="center", x=0.5))
        fig_comp.update_traces(textposition="outside", textfont_size=12, cliponaxis=False,
                               hovertemplate="<b>%{y}</b><br>%{text}<extra></extra>")
        ...
        _mp_diff = _mp_diff.merge(_moku_kuan, left_on="budget_category", right_on="moku", how="left")
        ...
        if not _big_kuan.empty:
            ...
            for _, bk in _big_kuan.iterrows():
                ...
                if reasons:
                    annotations.append(f"<b>{kname}</b>（{fmt_diff(int(bk['diff']), short=True)}）… {' / '.join(reasons)}")
            if annotations:
    ...
    with tab2:
        ...
        with col_rev1:
            ...
            fig_rev2 = px.pie(rev_by_class, values="amount_current", names="財源区分", color="財源区分",
                              color_discrete_map={"自主財源": "#22c55e", "依存財源": "#f97316"}, hole=0.5)
            fig_rev2.update_layout(height=360, margin=dict(l=10, r=10, t=10, b=10),
                                   legend=dict(orientation="h", yanchor="bottom", y=-0.15, xanchor="center", x=0.5, font_size=13))
            fig_rev2.update_traces(textinfo="percent", textfont_size=14, textposition="inside",
        ...
        with col_rev2:
            ...
            fig_rev3 = px.pie(rev_3cat, values="amount", names="区分", color="区分",
                              color_discrete_map={"自主財源(税等)": "#22c55e", "ふるさと納税": "#2563eb", "依存財源": "#f97316"}, hole=0.5)
            fig_rev3.update_layout(height=360, margin=dict(l=10, r=10, t=10, b=10),
                                   legend=dict(orientation="h", yanchor="bottom", y=-0.15, xanchor="center", x=0.5, font_size=13))
            fig_rev3.update_traces(textinfo="percent", textfont_size=14, textposition="inside",
                                   hovertemplate="<b>%{label}</b><br>%{percent}<br>%{value:,}千円<extra></extra>")
        ...
        rev_table = rev_table.rename(columns={"kuan": "款", "amount_current": "本年度(千円)",
        ...
        fig_nature = px.treemap(nature_tree, path=["分類", "性質"], values="金額", color="性質",
                                color_discrete_map=NATURE_COLORS)
        fig_nature.update_layout(height=420, margin=dict(l=10, r=10, t=10, b=10))
        fig_nature.update_traces(texttemplate="<b>%{label}</b><br>%{percentRoot:.1%}",
                                 hovertemplate="<b>%{label}</b><br>%{value:,}千円<br>%{percentRoot:.1%}<extra></extra>",
                                 textfont_size=12)
        ...
        nature_table["分類"] = nature_table["性質"].apply(
            lambda x: "義務的経費" if x in obligatory_cats else ("投資的経費" if x == "投資的経費" else "その他経費"))
        st.dataframe(nature_table[["分類", "性質", "金額表示", "構成比", "金額"]].rename(columns={"金額": "金額(千円)"}),
    ...
    with tab3:
        ...
        for col, name, color in [
            ...
            fig_src.add_trace(go.Bar(y=src_data["kuan"], x=src_data[col], name=name, orientation="h",
                                     marker_color=color,
                                     hovertemplate=f"<b>%{{y}}</b><br>{name}: %{{x:,}}千円<extra></extra>"))
        fig_src.update_layout(barmode="stack", height=max(350, len(src_data) * 36),
                              margin=dict(l=0, r=20, t=10, b=10), xaxis=dict(visible=False), yaxis=dict(title=""),
        ...
        norm = (log_amounts - a_min) / (a_max - a_min) if a_max > a_min else np.ones_like(log_amounts) * 0.5
        colors = [f"rgba(51,65,85,{0.15 + 0.70 * n:.2f})" for n in norm]

        fig_gen = go.Figure()
        fig_gen.add_trace(go.Bar(x=gen_ratio["一般財源充当率"].values, y=gen_ratio["kuan"].values,
                                  orientation="h", text=gen_ratio["label"].values, marker_color=colors,
                                  customdata=gen_ratio["金額表示"].values,
                                  hovertemplate="<b>%{y}</b><br>一般財源充当率: %{text}<br>%{customdata}<extra></extra>"))
        fig_gen.update_layout(height=max(300, len(gen_ratio) * 36), margin=dict(l=0, r=60, t=10, b=10),
                              xaxis=dict(visible=False, range=[0, 115]), yaxis=dict(title=""), showlegend=False)
        fig_gen.update_traces(textposition="outside", textfont_size=12, cliponaxis=False)
        ...
        fig_personnel = px.bar(personnel_chart, x="人件費", y="kuan_display", orientation="h",
                               text="label", color_discrete_sequence=["#2563eb"])
        fig_personnel.update_layout(height=max(300, len(personnel_chart) * 34),
                                    margin=dict(l=0, r=50, t=10, b=10),
                                    xaxis=dict(visible=False), yaxis=dict(title=""), showlegend=False)
        fig_personnel.update_traces(textposition="outside", textfont_size=12, cliponaxis=False)
        ...
        with st.expander("人件費の内訳（節別）"):
            ...
            st.dataframe(personnel_by_setsu[["節", "金額表示", "構成比", "金額"]].rename(columns={"金額": "金額(千円)"}),
        ...
        fig_bond = px.bar(bond_chart, x="src_bond", y="kuan_display", orientation="h",
                          text="label", color_discrete_sequence=["#f97316"])
        fig_bond.update_layout(height=max(200, len(bond_chart) * 40), margin=dict(l=0, r=50, t=10, b=10),
                               xaxis=dict(visible=False), yaxis=dict(title=""), showlegend=False)
        fig_bond.update_traces(textposition="outside", textfont_size=12, cliponaxis=False)
    ...
    with tab4:
        ...
        with mc1:
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">一般会計 主要事業数</div><div class="indicator-value">{mp_count}<span style="font-size:1rem">件</span></div></div>', unsafe_allow_html=True)
        with mc2:
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">うち新規事業</div><div class="indicator-value clr-blue">{mp_new_count}<span style="font-size:1rem">件</span></div></div>', unsafe_allow_html=True)
        with mc3:
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">主要事業 合計額</div><div class="indicator-value">{fmt_oku(mp_total)}</div></div>', unsafe_allow_html=True)
        ...
        for _, row in mp_new.iterrows():
            ...
            st.markdown(
                ...
                f'{desc_short}</div>', unsafe_allow_html=True)

        st.markdown('<div class="section-title">担当課別の事業数・予算額</div>', unsafe_allow_html=True)
        st.caption("どの課がどれくらいの事業と予算を担当しているか")

        dept_agg = mp.groupby("department").agg(事業数=("id", "count"), 新規=("is_new", "sum"),
                                                 予算額=("amount_current", "sum")).reset_index().rename(columns={"department": "担当課"})
        dept_agg["新規"] = dept_agg["新規"].astype(int)
        dept_agg = dept_agg.sort_values("予算額", ascending=False)

        fig_dept = px.bar(dept_agg.sort_values("予算額", ascending=True), x="予算額", y="担当課", orientation="h",
                          text=dept_agg.sort_values("予算額", ascending=True)["予算額"].apply(lambda x: fmt_oku(x, short=True)),
                          color_discrete_sequence=["#2563eb"])
        fig_dept.update_layout(height=max(300, len(dept_agg) * 34), margin=dict(l=0, r=50, t=10, b=10),
                               xaxis=dict(visible=False), yaxis=dict(title=""), showlegend=False)
        fig_dept.update_traces(textposition="outside", textfont_size=12, cliponaxis=False)
        ...
        st.dataframe(dept_table[["担当課", "事業数", "新規", "予算額表示", "予算額"]].rename(columns={"予算額": "予算額(千円)"}),
        ...
        with filter_col1:
            dept_filter = st.selectbox("担当課で絞り込み", ["すべて"] + sorted(mp["department"].unique().tolist()), key="mp_dept_filter")
        with filter_col2:
            new_filter = st.selectbox("新規/継続", ["すべて", "新規のみ", "継続のみ"], key="mp_new_filter")
        ...
        mp_show = mp_filtered[["project_name", "department", "budget_category", "amount_current", "amount_previous", "is_new", "description"]].rename(columns={
            "project_name": "事業名", "department": "担当課", "budget_category": "予算科目",
            "amount_current": "本年度(千円)", "amount_previous": "前年度(千円)", "is_new": "新規", "description": "説明"
    ...
    with tab5:
        ...
        mp_projects_ft = mp_furusato[~mp_furusato["project_name"].isin(["ふるさと応援寄附金基金積立金", "ふるさと応援寄附金事業"])].sort_values("furusato_amount", ascending=False)
        ...
        with fu1:
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">基金積立</div><div class="indicator-value">{fmt_oku(fund_amt)}</div><div class="indicator-sub">将来の事業のための貯蓄</div></div>', unsafe_allow_html=True)
        with fu2:
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">運営コスト</div><div class="indicator-value clr-red">{fmt_oku(ops_amt)}</div><div class="indicator-sub">返礼品・広告・事務費</div></div>', unsafe_allow_html=True)
        with fu3:
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">事業への充当</div><div class="indicator-value clr-green">{fmt_oku(projects_amt)}</div><div class="indicator-sub">{len(mp_projects_ft)}事業に活用</div></div>', unsafe_allow_html=True)

        THEME_MAP = {
            "子育て・教育": ["結婚祝金", "誕生祝金", "入学祝金", "卒業祝金", "障害児通所支援給付費", "母子保健事業委託料",
                          "小学校補助教材支給事業（消耗品費）", "中学校補助教材支給事業（消耗品費）",
                          "小学校高度情報教育用備品購入事業", "中学校高度情報教育用備品購入事業", "学校施設整備改修事業"],
            "健康・医療": ["各種健（検）診委託料", "定期予防接種委託料"],
            "農林水産業": ["鳥獣被害防止総合対策交付金事業", "有害鳥獣駆除対策費補助金", "有害鳥獣被害防止対策費補助金",
                        "親元就農給付金", "さが園芸888整備支援事業費補助金", "農地基盤整備事業費補助金",
                        "森林整備担い手育成基金助成事業費補助金", "森林環境保全直接支援事業委託料", "漁業従事者事業継続支援給付金"],
            "インフラ・環境": ["交通安全施設整備事業", "町道維持補修事業", "家庭用合併処理浄化槽設置整備事業費補助金", "防災ハザードマップ更新委託料"],
            "交通・移住": ["コミュニティバス運営事業", "タクシー運営事業", "移住定住促進事業補助金"],
            "観光・商工": ["観光客誘客事業補助金"],
        }
        project_to_theme = {}
        ...
        theme_colors = {"子育て・教育": "#2563eb", "健康・医療": "#22c55e", "農林水産業": "#f97316",
                        "インフラ・環境": "#06b6d4", "交通・移住": "#a855f7", "観光・商工": "#ec4899", "その他": "#94a3b8"}

        col_fu1, col_fu2 = st.columns([1, 1])
        with col_fu1:
            st.markdown('<div class="section-subtitle">寄附金の全体構成</div>', unsafe_allow_html=True)
            overview_data = pd.DataFrame([{"区分": "基金積立", "金額": fund_amt}, {"区分": "運営コスト", "金額": ops_amt}, {"区分": "事業充当", "金額": projects_amt}])
            fig_fu_overview = px.pie(overview_data, values="金額", names="区分", color="区分",
                                     color_discrete_map={"基金積立": "#94a3b8", "運営コスト": "#f43f5e", "事業充当": "#22c55e"}, hole=0.5)
            fig_fu_overview.update_layout(height=320, margin=dict(l=10, r=10, t=10, b=10))
            fig_fu_overview.update_traces(textinfo="label+percent", textfont_size=12,
                                          hovertemplate="<b>%{label}</b><br>%{percent}<br>%{value:,}千円<extra></extra>")
        ...
        with col_fu2:
            ...
            fig_fu_theme = px.bar(theme_bar, x="金額", y="テーマ", orientation="h", color="テーマ",
                                  color_discrete_map=theme_colors, text="label")
            fig_fu_theme.update_layout(height=max(250, len(theme_bar) * 40), margin=dict(l=10, r=50, t=10, b=10),
                                       showlegend=False, xaxis=dict(visible=False), yaxis=dict(title=""))
            fig_fu_theme.update_traces(textposition="outside", textfont_size=12, cliponaxis=False,
        ...
        fig_fu_bar = px.bar(mp_proj_chart, x="furusato_amount", y="project_name", orientation="h",
                            color="テーマ", color_discrete_map=theme_colors, text="label")
        fig_fu_bar.update_layout(height=max(350, len(mp_proj_chart) * 30), margin=dict(l=0, r=50, t=10, b=10),
        ...
        fig_fu_bar.update_traces(textposition="outside", textfont_size=11, cliponaxis=False)
        ...
        all_theme_agg = mp_themed.groupby("テーマ").agg(事業数=("id", "count"), 新規=("is_new", "sum"),
                                                         予算額=("amount_current", "sum")).reset_index()
        all_theme_agg["新規"] = all_theme_agg["新規"].astype(int)
        all_theme_agg = all_theme_agg.sort_values("予算額", ascending=False)

        fig_theme = px.bar(all_theme_agg.sort_values("予算額", ascending=True), x="予算額", y="テーマ", orientation="h",
                           color="テーマ", color_discrete_map=theme_colors,
                           text=all_theme_agg.sort_values("予算額", ascending=True)["予算額"].apply(lambda x: fmt_oku(x, short=True)))
        fig_theme.update_layout(height=max(250, len(all_theme_agg) * 40), margin=dict(l=0, r=50, t=10, b=10),
                                xaxis=dict(visible=False), yaxis=dict(title=""), showlegend=False)
        fig_theme.update_traces(textposition="outside", textfont_size=12, cliponaxis=False)
        ...
        st.dataframe(theme_table[["テーマ", "事業数", "新規", "予算額表示", "予算額"]].rename(columns={"予算額": "予算額(千円)"}),
                     use_container_width=True, hide_index=True, height=400)
        ...
        with fc1:
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">寄附金収入</div><div class="indicator-value clr-blue">{fmt_oku(furusato_rev)}</div></div>', unsafe_allow_html=True)
        with fc2:
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">運営コスト</div><div class="indicator-value clr-red">{fmt_oku(furusato_cost_total)}</div><div class="indicator-sub">コスト率 {safe_pct(furusato_cost_total, furusato_rev):.1f}%</div></div>', unsafe_allow_html=True)
        with fc3:
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">実質収入</div><div class="indicator-value clr-green">{fmt_oku(furusato_net)}</div><div class="indicator-sub">寄附金 − 運営コスト</div></div>', unsafe_allow_html=True)

        with st.expander("運営コストの内訳"):
            cost_df = pd.DataFrame([{"項目": k, "金額(千円)": v, "金額": fmt_oku(v)} for k, v in FURUSATO_COST_ITEMS.items()])
        ...
        sim_pct = st.slider("ふるさと納税の規模（現状比）", min_value=0, max_value=200, value=100, step=10,
                            format="%d%%", help="0%=ふるさと納税がゼロ、100%=現状維持、200%=2倍")
        ...
        with sc1:
            ...
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">歳入総額</div><div class="indicator-value">{sim_total_rev/100000:,.1f}<span style="font-size:0.9rem">億円</span></div><div class="indicator-sub" style="color:{rev_color}">現状比 {fmt_diff(rev_diff_sim)}</div></div>', unsafe_allow_html=True)
        with sc2:
            exp_diff_sim = sim_total_exp - total_exp
            exp_color = "#22c55e" if exp_diff_sim >= 0 else "#f43f5e"
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">歳出規模</div><div class="indicator-value">{sim_total_exp/100000:,.1f}<span style="font-size:0.9rem">億円</span></div><div class="indicator-sub" style="color:{exp_color}">現状比 {fmt_diff(exp_diff_sim)}</div></div>', unsafe_allow_html=True)
        with sc3:
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">町民1人あたり</div><div class="indicator-value">{sim_per_capita:,.0f}<span style="font-size:0.9rem">万円</span></div><div class="indicator-sub">現状 {per_capita_man:,.0f}万円</div></div>', unsafe_allow_html=True)

        sd1, sd2 = st.columns(2)
        with sd1:
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">自主財源比率</div><div class="indicator-value">{sim_indep_ratio:.1f}<span style="font-size:0.9rem">%</span></div><div class="indicator-sub">現状 {indep_total/total_rev*100:.1f}%</div></div>', unsafe_allow_html=True)
        with sd2:
            st.markdown(f'<div class="indicator-card"><div class="indicator-label">自主財源比率（ふるさと納税除く）</div><div class="indicator-value">{sim_indep_ex_ratio:.1f}<span style="font-size:0.9rem">%</span></div><div class="indicator-sub">この値はふるさと納税の規模に依らない町の実力値</div></div>', unsafe_allow_html=True)
        ...
        fig_compare = px.bar(compare_melted, x="金額", y="指標", color="区分", orientation="h", barmode="group",
                             color_discrete_map={"現状": "#94a3b8", "シミュレーション": "#2563eb"},
                             text=compare_melted["金額"].apply(lambda x: fmt_oku(x, short=True)))
        fig_compare.update_layout(height=300, margin=dict(l=0, r=50, t=10, b=10),
                                  xaxis=dict(visible=False), yaxis=dict(title=""),
                                  legend=dict(orientation="h", yanchor="bottom", y=-0.15, xanchor="center", x=0.5))
        fig_compare.update_traces(textposition="outside", textfont_size=12, cliponaxis=False)
        st.plotly_chart(fig_compare, use_container_width=True, config={"displayModeBar": False})

        if sim_pct == 0:
            st.markdown('<div class="highlight-box"><strong>ふるさと納税がゼロの場合</strong><br>・歳入が約10億円減少し、予算規模は大幅に縮小します<br>・一方で返礼品・広告費等の運営コスト（約4.7億円）も不要になります<br>・差引で約5.4億円の実質的な財源を失うことになります<br>・自主財源比率（ふるさと納税除く）は変わりません。これが「町の実力」です</div>', unsafe_allow_html=True)
        elif sim_pct < 100:
            loss = furusato_net - sim_net
            st.markdown(f'<div class="highlight-box"><strong>ふるさと納税が{sim_pct}%に減少した場合</strong><br>・実質収入が{fmt_oku(loss)}減少します<br>・運営コストも比例して減りますが、寄附金の減少分を補えません<br>・ふるさと納税以外の自主財源の強化が課題になります</div>', unsafe_allow_html=True)
        elif sim_pct > 100:
            gain = sim_net - furusato_net
            st.markdown(f'<div class="highlight-box"><strong>ふるさと納税が{sim_pct}%に増加した場合</strong><br>・実質収入が{fmt_oku(gain)}増加します<br>・ただしコスト率は約{safe_pct(furusato_cost_total, furusato_rev):.0f}%で一定と仮定しています。実際には広告効率の変化等で変動します<br>・規模拡大に伴う人件費・事務負担の増加にも注意が必要です</div>', unsafe_allow_html=True)
    ...
    with tab6:
        ...
        sa_colors = {"一般": "#3b82f6", "町立太良病院": "#f43f5e", "国民健康保険": "#f97316",
                     "後期高齢者医療": "#a855f7", "簡易水道": "#06b6d4", "漁業集落排水": "#22c55e", "水道": "#0ea5e9"}
        ...
        for _, row in sa_chart.iterrows():
            ...
            fig_sa.add_trace(go.Bar(
                x=[row["amount_current"]], y=[name], orientation="h", name=name,
                marker_color=color, text=[row["label"]], textposition="outside", textfont=dict(size=12),
        ...
        fig_sa.update_layout(height=max(250, len(sa_chart) * 45), margin=dict(l=0, r=70, t=10, b=10),
                             xaxis=dict(visible=False), yaxis=dict(title=""), showlegend=False, bargap=0.3)
        ...
        sa_table["増減表示"] = sa_table.apply(lambda r: f"{'+'if r['増減額']>0 else ''}{fmt_oku(abs(int(r['増減額'])))}", axis=1)
        sa_table["繰出表示"] = sa_table["一般会計繰出"].apply(lambda x: fmt_oku(x) if x > 0 else "-")

        total_row = pd.DataFrame([{
            "会計名": "総計", "R8年度表示": fmt_oku(sa_total_current), "R7年度表示": fmt_oku(sa_total_previous),
            "増減表示": f"+{fmt_oku(sa_total_diff)}" if sa_total_diff > 0 else fmt_oku(sa_total_diff),
            "増減率(%)": round(sa_total_diff / sa_total_previous * 100, 1), "構成比(%)": 100.0,
            "繰出表示": fmt_oku(sa_transfer_total), "増減の要因": ""}])
        ...
        with dn_col1:
            ...
            fig_sa_tree = px.treemap(sa_tree, path=["short_name"], values="amount_current", color="short_name",
                                     color_discrete_map=sa_tree_colors)
            fig_sa_tree.update_layout(height=380, margin=dict(l=10, r=10, t=10, b=10))
            fig_sa_tree.update_traces(texttemplate="<b>%{label}</b><br>%{value:,.0f}千円<br>%{percentRoot:.1%}",
                                      hovertemplate="<b>%{label}</b><br>%{value:,.0f}千円<br>%{percentRoot:.1%}<extra></extra>", textfont_size=12)
        ...
        with dn_col2:
            ...
            for _, r in sa_all.iterrows():
                ...
                st.markdown(
                    ...
                    f'<span style="font-size:0.82rem;color:#64748b">{note}</span></div>', unsafe_allow_html=True)
        ...
        for i, (_, row) in enumerate(sa_transfer.iterrows()):
            ...
            fig_transfer.add_trace(go.Bar(
                x=[row["transfer_from_general"]], y=[name], orientation="h", name=name,
                marker_color=sa_colors.get(name, "#94a3b8"), text=[row["label"]], textposition="outside", textfont=dict(size=12),
                hovertemplate=f"{row['account_name']}<br>繰出金: {fmt_oku(int(row['transfer_from_general']))}<br>会計予算の{row['ratio']:.1f}%<extra></extra>"))
        fig_transfer.update_layout(height=max(180, len(sa_transfer) * 50), margin=dict(l=0, r=50, t=10, b=10),
                                   xaxis=dict(visible=False), yaxis=dict(title=""), showlegend=False)
        ...
        if not mp_special.empty:
            ...
            mp_by_acct = mp_special.groupby("account_type").agg(事業数=("id", "count"), 合計額=("amount_current", "sum")).reset_index().sort_values("合計額", ascending=False)
            for _, sa_row in mp_by_acct.iterrows():
                ...
                lines = [f'<div class="highlight-box"><strong>{acct}</strong>']
                if budget_amt > 0:
    ...
    with tab7:
        ...
        selected_kuan = st.selectbox("見たい款を選んでください", sum_data.sort_values("kuan_no")["kuan"].tolist())
        ...
        if not kuan_detail.empty:
            ...
            with kc1:
                st.markdown(f'<div class="hero-card"><div class="hero-label">本年度</div><div class="hero-number" style="font-size:2rem">{fmt_oku(int(kuan_sum["amount_current"]))}</div></div>', unsafe_allow_html=True)
            with kc2:
                st.markdown(f'<div class="hero-card"><div class="hero-label">前年度</div><div class="hero-number" style="font-size:2rem; color:#64748b">{fmt_oku(int(kuan_sum["amount_previous"]))}</div></div>', unsafe_allow_html=True)
            with kc3:
                d = int(kuan_sum['diff'])
                dc = COLOR_INCREASE if d >= 0 else COLOR_DECREASE
                st.markdown(f'<div class="hero-card"><div class="hero-label">増減</div><div class="hero-number" style="font-size:2rem; color:{dc}">{fmt_diff(d)}</div></div>', unsafe_allow_html=True)
            ...
            if pd.notna(kuan_sum.get("src_national")):
                ...
                src_pie_data = pd.DataFrame([{"財源": k, "金額": v} for k, v in src_vals.items() if v > 0])
                if not src_pie_data.empty:
                    src_pie_colors = {"国・県支出金": "#2563eb", "地方債": "#f97316", "その他特定財源": "#22c55e", "一般財源": "#94a3b8"}
                    fig_src_pie = px.pie(src_pie_data, values="金額", names="財源", color="財源", color_discrete_map=src_pie_colors, hole=0.5)
                    fig_src_pie.update_layout(height=300, margin=dict(l=10, r=10, t=10, b=10),
                                              legend=dict(orientation="h", yanchor="bottom", y=-0.15, xanchor="center", x=0.5))
                    fig_src_pie.update_traces(textinfo="label+percent", textfont_size=12,
                                              hovertemplate="<b>%{label}</b><br>%{percent}<br>%{value:,}千円<extra></extra>")
                    st.plotly_chart(fig_src_pie, use_container_width=True, config={"displayModeBar": False})

            st.markdown("")

            moku_agg = kuan_detail.groupby(["kou", "moku"]).agg(amount_current=("amount_current", "first")).reset_index().drop_duplicates().sort_values("amount_current", ascending=False)
            ...
            show_moku = moku_agg.head(15).copy().sort_values("amount_current", ascending=True)
            show_moku["label"] = show_moku["amount_current"].apply(lambda x: fmt_oku(x, short=True))

            fig_moku = px.bar(show_moku, x="amount_current", y="moku", orientation="h", color="kou",
                              labels={"moku": "", "kou": "項"}, text="label")
            fig_moku.update_layout(height=max(300, len(show_moku) * 38), margin=dict(l=0, r=50, t=10, b=10),
                                   xaxis=dict(visible=False), yaxis=dict(title=""),
                                   legend=dict(orientation="h", yanchor="bottom", y=-0.2, font_size=13))
            fig_moku.update_traces(textposition="outside", textfont_size=12, cliponaxis=False,
                                   hovertemplate="<b>%{y}</b><br>%{text}<extra></extra>")
            ...
            rename_map = {"kou": "項", "moku": "目", "setsu": "節", "setsu_amount": "金額(千円)", "src_general": "一般財源", "description": "説明"}
            ...
            if kuan_prev > 0:
                ...
                if abs(kuan_change_pct) >= 5:
                    ...
                    check_items.append(("change", _subtype,
            ...
            moku_detail = kuan_detail.groupby(["kou", "moku"]).agg(
                amount_current=("amount_current", "first"), amount_previous=("amount_previous", "first")).reset_index().drop_duplicates()
            ...
            src_bond_val = int(kuan_sum["src_bond"]) if pd.notna(kuan_sum.get("src_bond")) else 0
            kuan_total = int(kuan_sum["amount_current"])
            if kuan_total > 0:
                gen_ratio_val = src_gen / kuan_total * 100
                if gen_ratio_val >= 80:
                    check_items.append(("source", "high_general",
                        f"一般財源充当率が<b>{gen_ratio_val:.0f}%</b>と高く、ほぼ町の自主財源で賄っています。"))
                if src_bond_val > 0:
                    bond_ratio_val = src_bond_val / kuan_total * 100
                    check_items.append(("source", "bond",
                        f"地方債を<b>{fmt_oku(src_bond_val)}</b>（{bond_ratio_val:.0f}%）充当しています。"))
                if src_nat > 0 and src_nat / kuan_total * 100 >= 20:
                    check_items.append(("source", "national",
                        f"国・県支出金が<b>{fmt_oku(src_nat)}</b>（{safe_pct(src_nat, kuan_total):.0f}%）を占めています。"))

            big_setsu = kuan_detail[kuan_detail["setsu_amount"].notna() & (kuan_detail["setsu_amount"] >= 50000)].sort_values("setsu_amount", ascending=False).head(3)
            for _, s in big_setsu.iterrows():
                ...
                check_items.append(("detail", "large",
            ...
            if check_items:
                ...
                for tag, subtype, text in check_items:
                    ...
                    items_html += f'<div class="check-item"><span class="tag {tag_class}">{tag_label}</span>{text}'
                    if ctx:
                        why, question = ctx
                        items_html += f'<br><span style="font-size:0.8rem;color:#64748b">💡 {why}</span>'
                        items_html += f'<br><span style="font-size:0.8rem;color:#3b82f6">❓ 質問例：{question}</span>'
                    items_html += '</div>'
                st.markdown('<div class="check-box"><div class="check-box-title">分析メモ</div>' + items_html + '</div>', unsafe_allow_html=True)
            else:
        ...
        if search_query:
            search_text = (detail["kuan"].fillna("") + " " + detail["kou"].fillna("") + " " +
                           detail["moku"].fillna("") + " " + detail["setsu"].fillna("") + " " +
                           detail["description"].fillna("")).str.lower()
        ...
        rename_all = {"kuan": "款", "kou": "項", "moku": "目", "setsu": "節", "setsu_amount": "金額(千円)", "description": "説明"}
